...
- **로컬 파일·링크**: [`BK MES.docx`](BK%20MES.docx) — `docs/`에 두면 [`HANDOVER_NEW_DEVELOPER.html`](HANDOVER_NEW_DEVELOPER.html)과 같은 폴더 기준으로 브라우저에서 다운로드·열기(팀 정책에 따름).  
- **의도된 경로**: `docs/BK MES.docx` (팀에서 배포하는 Word 원본이 있을 경우 이 위치에 두면 찾기 쉽다.)  
- **현재 저장소**: 동일 경로에 파일이 **있을 수 있다**. 없으면 사내 공유 드라이브·메일 등에서 확보 후 위 경로에 복사한다.  
